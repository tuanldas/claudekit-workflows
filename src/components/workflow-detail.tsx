                ...
                {phase.steps.map((step, stepIdx) => (
                  <div key={stepIdx}>
                    <div className="flex items-start gap-1.5">
                      <code className="shrink-0 font-mono text-micro text-foreground">
                        {step.command}
                      </code>
                      {step.optional && (
                    ...
                    <p className="mt-0.5 text-micro leading-relaxed text-foreground-muted">
        ...
        <div className="mt-4 border-t border-border pt-4">
          ...
          <ul className="flex flex-col gap-1.5">
            {workflow.tips.map((tip, i) => (
              <li
                key={i}
                className="flex items-start gap-2 text-caption leading-relaxed text-foreground-muted"
              >
                <span
                  aria-hidden
...
function CloseButton({ onClose }: { onClose: () => void }) {
  return (
    <button
      onClick={onClose}
      aria-label="Close"
      className="inline-flex size-7 shrink-0 cursor-pointer items-center justify-center rounded-[var(--radius-sm)] text-foreground-muted transition-colors hover:bg-surface-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-ring)]"
    >
      <svg
        width="16"
        height="16"
